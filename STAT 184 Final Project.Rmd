...
youtube_data_grouped <- GlobalYouTube %>%
  ...
  summarize(Channel_Count = n(),
            Average_Subscribers = mean(V3, na.rm = TRUE))

youtube_data_grouped %>%
  ggplot(aes(x=reorder(V8, Channel_Count), y=Channel_Count)) +
  geom_col(fill="navy") + 
...
The p-value for the Internet speed coefficient is 0.14, which is above common levels of significance, indicating that the effect of Internet speed on the number of YouTube subscribers is not statistically significant.

R-squared = 0.004728. This indicates that the Internet speed in the model explains a very low proportion (less than 0.5%) of the variation in the number of YouTube subscribers. In other words, the variation in the number of YouTube subscribers can hardly be explained by Internet speed.
